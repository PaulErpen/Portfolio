...
class MtgProxyView(Frame):
    ...
    Attributes:
        ...
        cardNum: the number of unique cards which have been added
        imgPaths: the paths of all the images which have been added
    ...
    def data(self):
        ...
        for i in range(self.cardNum):
            ...
            pilFile = Image.open(self.imgPaths[i])
            ...
            #other labels
            Label(self.frame,text="Card is being printed "+str(self.cardHowOften[i])+" times.").grid(row=i,column=1)
        ...
        self.addButton = Button(self.frame, text="add a new Card", command = self.addNewCard)
        self.addButton.grid(row=self.cardNum, column = 0, columnspan = 2, padx = 10, pady= 20, sticky="W")
    ...
    def delete(self,i):
        ...
        if self.cardNum > i:
            for j in range(i, self.cardNum):
                self.cardHowOften[j]=self.cardHowOften[j+1]
                self.imgPaths[j]=self.imgPaths[j+1]
        self.cardHowOften[self.cardNum] = 0
        self.imgPaths[self.cardNum] = self.defaultImage
    ...
    def incrHowOften(self,i):
        ...
        """
        self.cardHowOften[i] = self.cardHowOften[i]+1
        self.updateOne(i)
    ...
    def addNewCard(self):
        ...
        """
        self.cardNum = self.cardNum +1
        self.imgPaths.insert(self.cardNum,self.defaultImage)
    ...
    def makePdf(self):
        ...
        if name is not None:
            proxymaker.writeData(self.cardNum,self.imgPaths,self.cardHowOften,self.safeHome+"/"+name+".pdf")
